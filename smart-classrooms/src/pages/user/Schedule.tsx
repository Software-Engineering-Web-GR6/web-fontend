...
const UserSchedule: React.FC = () => {
    ...
    useEffect(() => {
        const load = async () => {
            ...
            try {
                ...
                setError(typeof detail === "string" ? detail : "Khong the tai thoi khoa bieu cua ban.");
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <Layout
            ...
            subtitle="Lich hoc ca nhan theo tuan cho nguoi dung"
            isAdmin={false}
        >
            {error && (
            ...
            {loading ? (
                <Card className="rounded-3xl">
                    <div className="py-10 text-center text-slate-500">Dang tai thoi khoa bieu...</div>
                </Card>
            ) : accesses.length === 0 ? (
                <Card className="rounded-3xl border-dashed py-12 text-center text-slate-500">
                    Ban chua co tiet hoc nao trong thoi khoa bieu.
                </Card>
            ) : (
                <>
                    <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-3">
                        <Card className="rounded-3xl" padding="sm">
                            <div className="flex items-center justify-between gap-3">
                                <div>
                                    <p className="text-sm text-slate-500">Tong tiet moi tuan</p>
                        ...
                        <Card className="rounded-3xl" padding="sm">
                            <div className="flex items-center justify-between gap-3">
                                <div>
                                    <p className="text-sm text-slate-500">Tiet hom nay</p>
                        ...
                        <Card className="rounded-3xl" padding="sm">
                            <div className="flex items-center justify-between gap-3">
                                <div>
                                    <p className="text-sm text-slate-500">Ca hien tai</p>
                                    <p className="text-2xl font-bold text-slate-900">
                                        {currentShift ? `Ca ${currentShift}` : "Ngoai khung hoc"}
                                    </p>
                                </div>
                    ...
                    <Card className="rounded-3xl">
                        <CardHeader>
                            <CardTitle>Lich hoc theo tuan</CardTitle>
                        </CardHeader>
                        ...
                        <div className="mb-4 flex flex-wrap gap-2 md:hidden">
                            {DAYS.map((day) => (
                                <button
                                    ...
                                    className={`rounded-xl border px-3 py-1.5 text-xs font-semibold ${selectedDay === day.value
                        ...
                        <div className="grid gap-3 md:hidden">
                            {selectedDayEntries.length === 0 ? (
                                <div className="rounded-2xl border border-dashed border-slate-200 px-4 py-8 text-center text-sm text-slate-500">
                                    Khong co tiet hoc trong ngay da chon.
                                </div>
                            ) : (
                                selectedDayEntries.map((entry) => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`rounded-2xl border px-4 py-3 ${isNow
                                        ...
                                        >
                                            ...
                                            <p className="mt-2 text-sm font-medium text-indigo-700">
                                                {room ? getRoomLabel(room) : `Phong ${entry.room_id}`}
                                            </p>
                        ...
                        <div className="hidden overflow-hidden rounded-3xl border border-slate-200 md:block">
                            <div className="grid grid-cols-[92px_repeat(7,minmax(0,1fr))] bg-white">
                                ...
                                {DAYS.map((day) => (
                                    <div
                                        key={day.value}
                                        className={`border-b border-slate-200 px-2 py-3 text-center ${day.value === currentDay ? "bg-indigo-50" : "bg-slate-50"
                                            }`}
                                    >
                                        ...
                                        {DAYS.map((day) => {
                                            ...
                                            return (
                                                <div
                                                    ...
                                                    className={`min-h-[110px] border-t border-slate-100 p-2 ${slot
                                                        ? "bg-emerald-50/50"
                                                        : "bg-white"
                                                    ...
                                                    {slot ? (
                                                        <div className="h-full rounded-2xl border border-emerald-200 bg-white px-2 py-2">
                                                            <p className="text-xs font-semibold text-slate-900">
                                                                {slot.room ? getRoomLabel(slot.room) : `Phong ${slot.access.room_id}`}
                                                            </p>
                                                            <p className="mt-1 text-[11px] text-slate-500">
                                                                {slot.room?.location || "Da xep trong thoi khoa bieu"}
                                                            </p>
                                                            {isNow && (
