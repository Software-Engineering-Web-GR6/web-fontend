...
export const HumidityChart: React.FC<HumidityChartProps> = ({
  ...
  const minValue = values.length ? Math.min(...values) : 35;
  const maxValue = values.length ? Math.max(...values) : 70;
  const padding = 3;
  const yDomain: [number, number] = [
    Math.max(20, Math.floor(minValue - padding)),
    Math.min(95, Math.ceil(maxValue + padding)),
  ];
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <AreaChart data={data} margin={{ top: 12, right: 22, bottom: 8, left: 2 }}>
        <defs>
          <linearGradient id="humidityAreaFill" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor={CHART_COLORS.humidity} stopOpacity={0.35} />
            <stop offset="100%" stopColor={CHART_COLORS.humidity} stopOpacity={0.04} />
          </linearGradient>
        </defs>
        ...
        <ReferenceLine y={40} stroke="#10b981" strokeDasharray="3 3" ifOverflow="extendDomain" />
        <ReferenceLine y={60} stroke="#f97316" strokeDasharray="3 3" ifOverflow="extendDomain" />
        ...
        <Tooltip
          ...
          formatter={(value) => [`${Number(value).toFixed(1)}%`, "Độ ẩm"]}
        />
